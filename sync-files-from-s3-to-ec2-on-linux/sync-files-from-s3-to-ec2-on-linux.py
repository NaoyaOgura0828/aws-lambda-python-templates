...
def lambda_handler(event, context):
    ...
    linux_user_and_group_name = os.environ['LINUX_USER_AND_GROUP_NAME']
    ...
    try:
        ...
        # オーナー, グループ変更有
        if linux_user_and_group_name:
            chown_command = 'chown ' + linux_user_and_group_name + ':' + \
                linux_user_and_group_name + ' -R ' + target_file_path
            # Command実行
            ssm.send_command(
                InstanceIds=[instance_id],
                DocumentName='AWS-RunShellScript',
                Parameters={
                    'commands': [s3_sync_command, chown_command],
